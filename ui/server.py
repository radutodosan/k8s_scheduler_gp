...
def _list_experiment_sweeps() -> List[Dict[str, Any]]:
    ...
    for d in sorted(exp_dir.iterdir(), reverse=True):
        ...
        if analysis_dir.exists():
            info["analysis"] = {}
            for txt in ("analysis_report.txt", "statistical_report.txt"):
                p = analysis_dir / txt
                if p.exists():
                    info["analysis"][txt] = p.read_text(encoding="utf-8")
            # plots (as relative paths)

...
PROFILES: Dict[str, Dict[str, Any]] = {
    "web_serving": {
        ...
        "anti_affinity_probability": 0.25,
    ...
    "batch_processing": {
        "description": "Data processing / ETL jobs — heavy CPU/MEM, constant rate, low priority",
        "cpu_range": [1.0, 4.0],
        "mem_range": [2048.0, 8192.0],
    ...
    # ── Chaos / stress-testing profiles ──────────────────────────────────

    "spike": {
        "description": "Sudden traffic spike — massive burst of short-lived pods overwhelming the scheduler",
        "cpu_range": [0.2, 2.0],
        "mem_range": [256.0, 2048.0],
        "duration_range": [2.0, 10.0],
        "priority_weights": {"low": 0.2, "medium": 0.5, "high": 0.3},
        "qos_weights": {"best_effort": 0.5, "burstable": 0.3, "guaranteed": 0.2},
        "namespaces": ["spike-test", "load-test"],
        "arrival_pattern": "bursty",
        "bursty_spike_multiplier": 15.0,
        "bursty_spike_probability": 0.5,
        "anti_affinity_probability": 0.0,
        "possible_anti_affinity_keys": ["spike-pod"],
        "limit_probability": 0.3,
        "limit_ratio_min": 1.0,
        "limit_ratio_max": 3.0,
        "replica_group_probability": 0.0,
        "replica_group_size_range": [1, 1],
    },
    "gradual_ramp": {
        "description": "Gradually increasing load — simulates organic traffic growth until cluster saturation",
        "cpu_range": [0.5, 3.0],
        "mem_range": [512.0, 4096.0],
        "duration_range": [15.0, 60.0],
        "priority_weights": {"low": 0.3, "medium": 0.4, "high": 0.3},
        "qos_weights": {"best_effort": 0.3, "burstable": 0.4, "guaranteed": 0.3},
        "namespaces": ["ramp-test", "staging"],
        "arrival_pattern": "diurnal",
        "anti_affinity_probability": 0.3,
        "possible_anti_affinity_keys": ["ramp-svc"],
        "limit_probability": 0.5,
        "limit_ratio_min": 1.0,
        "limit_ratio_max": 2.0,
        "replica_group_probability": 0.3,
    ...
    "chaos_monkey": {
        "description": "Mixed heterogeneous pods with high resource variance — tests scheduler robustness under chaotic conditions",
        "cpu_range": [0.1, 6.0],
        "mem_range": [64.0, 16384.0],
        "gpu_range": [0.0, 2.0],
        "gpu_probability": 0.3,
        "duration_range": [1.0, 120.0],
        "priority_weights": {"low": 0.33, "medium": 0.34, "high": 0.33},
        "qos_weights": {"best_effort": 0.34, "burstable": 0.33, "guaranteed": 0.33},
        "namespaces": ["chaos", "canary", "experiment"],
        "arrival_pattern": "bursty",
        "bursty_spike_multiplier": 10.0,
        "bursty_spike_probability": 0.3,
        "anti_affinity_probability": 0.5,
        "possible_anti_affinity_keys": ["chaos-app", "canary-svc"],
        "limit_probability": 0.7,
        "limit_ratio_min": 1.0,
        "limit_ratio_max": 3.0,
        "possible_taints": ["gpu", "spot", "dedicated", "experimental"],
        "taint_toleration_probability": 0.4,
        "replica_group_probability": 0.2,
        "replica_group_size_range": [2, 6],
    },
}
